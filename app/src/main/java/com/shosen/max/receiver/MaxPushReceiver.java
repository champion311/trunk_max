package com.shosen.max.receiver;
...
public class MaxPushReceiver extends BroadcastReceiver {

    private static final String TAG = "MaxPushReceiver";
...
//                context.startActivity(i);
    ...
    private void processCustomMessage(Context context, Bundle bundle) {
        ...
        NewMessageBean bean = null;
        try {
            bean = new Gson().fromJson(message, NewMessageBean.class);
        } catch (Exception e) {
            e.printStackTrace();
            return;
        }
        if (CircleWrapperFragment.isForeground) {
            try {
        ...
        SPUtils.getInstance(Contstants.SP_NEW_MESSAGE).put(Contstants.NEW_MESSAGE_COUNT, bean.getNoReadNum());
    }
}
